...
6. 在写入 `matched_routes.txt` 前，做一次环路清理：只要出现 `A ... A` 闭环，就折叠为单个 `A`，去掉中间整段冗余路径。
...
其中每条成功轨迹在写出前会执行一次环路清理：

- 识别当前路径中再次出现的边 `X`（形成 `X ... X` 闭环）
- 删除两次 `X` 之间的整段中间边，只保留第一次出现的 `X`
- 该规则适用于任意长度环路（例如 `X Y X`、`X Y Z W X`）
...
此外，运行汇总里会额外给出环路清理相关统计：

- `with loops cleaned`：发生过至少一次环路清理的轨迹条数
- `Loops removed`：总共清理掉的环路次数
...
需要注意的是：这条规则针对的是输入匹配层，不会在匹配前去重。匹配完成后，输出阶段会额外清理任意长度的 `A ... A` 闭环。
