"""Training and evaluation engine for R-only STFGNN.
...
from reimplementation.stfgnn.graph import TEST_ONLY_TEMPORAL_GRAPH
from reimplementation.stfgnn.losses import HuberRawLoss
from reimplementation.stfgnn.model.fusion_graph import make_test_only_temporal_adjacency
from reimplementation.stfgnn.model.stfgnn import CODE_VERSION, STFGNN
from reimplementation.stfgnn.validation import (
    load_node_ids,
...
def test_only_temporal_adjacency(n_nodes: int) -> np.ndarray:
    """Deterministic in-memory graph. Never used as the official DTW matrix."""
    extra = tuple((index, (index + 3) % n_nodes) for index in range(0, n_nodes, 7) if index != (index + 3) % n_nodes)
    return make_test_only_temporal_adjacency(n_nodes, extra_pairs=extra)
...
def build_model(
    ...
    *,
    temporal_graph_is_test_only: bool,
) -> STFGNN:
    return STFGNN(
        spatial_adj,
        temporal_adj,
        ...
        filters=list(config.get("filters", [[64, 64, 64]] * 3)),
        ...
        temporal_graph_is_test_only=temporal_graph_is_test_only,
    )


def build_optimizer(model: STFGNN, config: Mapping[str, Any]) -> Adam:
    if str(config.get("optimizer", "adam")).lower() != "adam":
        raise ReimplementationError("original STFGNN training uses Adam")
    return Adam(
        model.parameters(),
...
def _gradient_flags(model: STFGNN) -> dict[str, bool]:
    flags = {
        "input_projection": False,
        "gcn": False,
        "stfgcm": False,
        "gated_cnn_filter": False,
        "gated_cnn_gate": False,
        "embedding": False,
        "mask": False,
        "output": False,
        "fusion_residual_path": False,
    }
    for name, tensor in model.named_parameters():
        ...
        if "gcn_layers" in name:
            flags["gcn"] = True
            flags["stfgcm"] = True
        if "filter_conv" in name:
            flags["gated_cnn_filter"] = True
            flags["fusion_residual_path"] = True
        if "gate_conv" in name:
            flags["gated_cnn_gate"] = True
            flags["fusion_residual_path"] = True
...
def run_p70_smoke(
    ...
    *,
    ...
    temporal_is_test_only = bool(graph_info.get("temporal_graph_is_test_only", False))
    model = build_model(
        config,
        graph_info["spatial_topology"],
        graph_info["temporal_adjacency"],
        temporal_graph_is_test_only=temporal_is_test_only,
    ).to(device)
    ...
    payload = {
        ...
        "model_structure": model_structure_payload(model),
        "init_parameter_sha256": init_hash,
        "parameters_changed": changed,
        "gradient_status": flags,
        "temporal_graph_kind": TEST_ONLY_TEMPORAL_GRAPH if temporal_is_test_only else "training_days_full_flow_dtw",
        "discarded_after_smoke": True,
        "did_not_write_official_checkpoint": True,
        "did_not_read_validation_or_test": True,
        "did_not_write_official_temporal_graph": True,
    ...
    print(
        f"[stfgnn] stage=smoke_testing rate=p70 loss={payload['training_loss']:.6f} status=ok",
        flush=True,
    )
    del model
    del optimizer
...
def run_single_rate(
    ...
    *,
    ...
    if bool(graph_info.get("temporal_graph_is_test_only")):
        raise ReimplementationError("official training cannot use test_only_temporal_adjacency")
    tag = rate_tag(rate)
    seed = int(config["seed"])
    seed_everything(seed)
    device = resolve_device(str(config.get("device", "auto")))
    ...
    model = build_model(
        ...
        graph_info["spatial_topology"],
        graph_info["temporal_adjacency"],
        temporal_graph_is_test_only=False,
    ).to(device)
    init_hash = parameter_sha256(model)
    optimizer = build_optimizer(model, config)
    ...
    def checkpoint_payload(epoch: int, metric: float) -> dict[str, Any]:
        return {
            ...
            "temporal_sha256": graph_info["temporal_sha256"],
            "data_file_sha256": train_hash,
    ...
    print(
        (
            f"[stfgnn] stage=building_model rate={tag} params={model.parameter_count()} "
    ...
    for epoch in range(start_epoch, epochs):
        ...
        logger.log({"stage": "epoch", "model_name": "stfgnn", "penetration_rate": tag, **row})
        print(
            (
                f"[stfgnn] stage=epoch rate={tag} epoch={epoch} "
...
def run_all_rates(
    ...
    *,
    ...
    graph_preview = {
        ...
        if key not in {"spatial_topology", "temporal_adjacency", "localized_adj", "mask_init", "node_ids"}
    }
    ...
    summaries = []
    for rate in rates:
        print(f"[stfgnn] stage=training_start rate={rate_tag(rate)} seed={config['seed']}", flush=True)
