"""PyTorch STSGCN matching ``reference/STSGCN/models/stsgcn.py`` plus the
input projection in ``reference/STSGCN/utils.py::construct_model``.
"""
...
from reimplementation.stsgcn.model.graph import (
    ...
    time_length_after_stsgcl,
)
from reimplementation.stsgcn.model.layers import OutputLayer, STSGCL, apply_mxnet_xavier

CODE_VERSION = "0.1.0"


class STSGCN(nn.Module):
    """R-only STSGCN. Public tensors are ``[B, T, V, C]``. Output is ``[B, 1, V, 1]``."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if filters is None:
            filters = [[64, 64, 64], [64, 64, 64], [64, 64, 64], [64, 64, 64]]
        ...
        self.xavier_magnitude = float(xavier_magnitude)
        self.n_stsgcl = len(self.filter_list)
        self.final_time_length = time_length_after_stsgcl(self.seq_len, self.n_stsgcl)
        if self.final_time_length < 1:
            raise ValueError(
                f"seq_len {self.seq_len} is too short for {self.n_stsgcl} STSGCL layers"
            )
        if spatial_adj.shape != (self.num_nodes, self.num_nodes):
            raise ValueError(
                f"spatial adjacency {spatial_adj.shape} != ({self.num_nodes}, {self.num_nodes})"
            )

        localized = construct_localized_adjacency(spatial_adj, steps=LOCAL_STEPS)
        self.register_buffer("localized_adj", torch.tensor(localized, dtype=torch.float32))
        if self.use_mask:
            mask = mask_init_from_localized_adjacency(localized)
        ...
        for group in self.filter_list:
            ...
            time_length -= 2
            channels = group[-1]
        self.stsgcl_layers = nn.ModuleList(stsgcl_layers)

        # Original builds ``predict_length`` independent heads, each with
        # ``predict_length=1``, then concatenates on the time axis. Horizon=1
        # keeps that structure with a single head.
        ...
        apply_mxnet_xavier(self, magnitude=self.xavier_magnitude, skip=("adj_mask",))
    ...
    def forward(self, data: torch.Tensor, return_trace: bool = False) -> torch.Tensor | tuple[torch.Tensor, dict[str, Any]]:
        ...
        for layer in self.stsgcl_layers:
