...
TrafficFlow 的主要代码和数据位于 `analysis/`。该目录把整个流水线按"阶段 + 数据形态"拆成 6 个模块：

1. `analysis/road_network/` —— 路网（OSM / SUMO net）
2. `analysis/ocr/` —— 截图 OCR → 原始 OSM 轨迹
...
除非特别说明，下文中的模块与文件路径都以 `analysis/` 为根目录。例如 `road_network/` 实际对应 `analysis/road_network/`。
...
sumo -n analysis/road_network/net_tls.net.xml \
  ...
  --fcd-output analysis/simulation/output/fcd.csv \
  --fcd-output.geo \
  --device.fcd.period 1 \
  --fcd-output.attributes id,lane,pos,speed,x,y \
...
sumo-gui -c simulation_replay.sumocfg \
