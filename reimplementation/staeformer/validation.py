"""Runtime checks for R-only STAEformer. The model does not load a road graph."""
...
from reimplementation.staeformer.model.staeformer import STAEformer, graph_keys_in_state_dict
...
def validate_no_graph_config(config: Mapping[str, Any]) -> None:
    ...
    if present:
        raise ReimplementationError(
            f"STAEformer config must not supply graph inputs {present}; "
...
def validate_model_shapes(
    ...
    *,
    ...
    expected = (int(batch_size), model.out_steps, model.num_nodes, model.output_dim)
    ...
    if int(model.model_dim) % int(model.num_heads) != 0:
        raise ReimplementationError("model_dim is not divisible by num_heads")
    return {
