"""Train R-only DCRNN. Does not write into prepared data, graphs, or original DCRNN."""
...
from reimplementation.dcrnn.engine import atomic_write_csv_rows, resolve_path, run_all_rates
from reimplementation.dcrnn.model.dcrnn_model import CODE_VERSION

SCRIPT_PATH = Path(__file__).resolve()
PROJECT_ROOT = SCRIPT_PATH.parents[2]
DEFAULT_CONFIG = SCRIPT_PATH.parent / "configs" / "r_only_dcrnn.json"

ORIGINAL_TO_PYTORCH = {
    "reference/dcrnn/lib/utils.py::calculate_random_walk_matrix": "reimplementation/dcrnn/graph.py::calculate_random_walk_matrix",
    "reference/dcrnn/model/dcrnn_cell.py::DCGRUCell.__init__ supports": "reimplementation/dcrnn/graph.py::build_dcrnn_supports",
    "reference/dcrnn/model/dcrnn_cell.py::_gconv": "reimplementation/dcrnn/model/diffusion_conv.py::build_diffusion_features",
    "reference/dcrnn/model/dcrnn_cell.py::DCGRUCell": "reimplementation/dcrnn/model/dcgru_cell.py::DCGRUCell",
    "reference/dcrnn/model/dcrnn_cell.py reset/update gates": "DCGRUCell gate_linear + sigmoid split",
    "reference/dcrnn/model/dcrnn_cell.py candidate": "DCGRUCell candidate_linear + tanh",
    "reference/dcrnn/model/dcrnn_cell.py projection": "DCGRUCell.proj_w (no bias)",
    "reference/dcrnn/model/dcrnn_model.py encoder MultiRNNCell": "reimplementation/dcrnn/model/encoder.py::DCRNNEncoder",
    "reference/dcrnn/model/dcrnn_model.py decoder MultiRNNCell + projection cell": "reimplementation/dcrnn/model/decoder.py::DCRNNDecoder",
    "reference/dcrnn/model/dcrnn_model.py GO_SYMBOL": "zeros [B, N * output_dim]",
    "reference/dcrnn/model/dcrnn_model.py scheduled sampling": "DCRNNDecoder._loop_input; unused for kept output when horizon=1",
    "reference/dcrnn/model/dcrnn_model.py seq2seq outputs[:-1]": "keep decoder steps [:horizon]",
    "reference/dcrnn/lib/metrics.py::masked_mae_loss": "reimplementation/dcrnn/losses.py::MaskedMAERawLoss",
    "reference/dcrnn/model/dcrnn_supervisor.py Adam": "torch.optim.Adam(eps=1e-3)",
    "reference/dcrnn/model/dcrnn_supervisor.py clip_by_global_norm": "torch.nn.utils.clip_grad_norm_",
    "reference/dcrnn/model/dcrnn_supervisor.py lr schedule": "reimplementation/dcrnn/engine.py::learning_rate_for_epoch",
    "tf.contrib.layers.xavier_initializer": "torch.nn.init.xavier_uniform_",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train R-only DCRNN (PyTorch port of original TF DCRNN).")
    ...
    parser.add_argument("--rnn-units", dest="rnn_units", type=int, default=None)
    parser.add_argument("--num-rnn-layers", dest="num_rnn_layers", type=int, default=None)
    parser.add_argument("--max-diffusion-step", dest="max_diffusion_step", type=int, default=None)
    parser.add_argument("--filter-type", dest="filter_type", type=str, default=None)
    parser.add_argument("--cl-decay-steps", dest="cl_decay_steps", type=int, default=None)
...
def original_hashes(project_root: Path) -> dict[str, str]:
    files = [
        "reference/dcrnn/lib/utils.py",
        "reference/dcrnn/lib/metrics.py",
        "reference/dcrnn/model/dcrnn_cell.py",
        "reference/dcrnn/model/dcrnn_model.py",
        "reference/dcrnn/model/dcrnn_supervisor.py",
        "reference/dcrnn/scripts/gen_adj_mx.py",
    ]
...
def main() -> None:
    ...
    if int(config.get("seq_len", 0)) != 12:
        raise ReimplementationError("R-only DCRNN seq_len must be 12")
    if bool(config.get("amp")):
        raise ReimplementationError("AMP is not part of the original DCRNN trainer")
    ...
    protected = [
        ...
        PROJECT_ROOT / "reference" / "dcrnn" / "lib" / "utils.py",
    ...
    atomic_write_json(output_root / "original_to_pytorch_mapping.json", ORIGINAL_TO_PYTORCH)
    atomic_write_json(
        output_root / "environment.json",
        {
            ...
            "command": sys.argv,
            "original_dcrnn_sha256": original_hashes(PROJECT_ROOT),
        },
    )
    ...
    print(
        f"[dcrnn] stage=scanning output_root={output_root.as_posix()} rates={config['rates']} device={config.get('device')}",
        flush=True,
    )
    try:
        ...
        logger.log({"stage": "completed", "model_name": "dcrnn", "status": "ok"})
        print("[dcrnn] stage=completed status=ok", flush=True)
    except Exception as exc:
        logger.log({"stage": "failed", "model_name": "dcrnn", "status": "failed", "latest_error": str(exc)})
        raise
    finally:
        logger.close()
