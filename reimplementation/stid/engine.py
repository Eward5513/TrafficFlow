"""Training engine for R-only STID.
...
from reimplementation.stid.data import assemble_history_features
from reimplementation.stid.losses import MAERawLoss
from reimplementation.stid.model.stid import CODE_VERSION, STID
from reimplementation.stid.temporal_identity import (
    ORIGINAL_DAY_SOURCE,
    day_of_week_index,
    load_weekday_mapping,
)
from reimplementation.stid.validation import (
    load_node_ids,
...
def _day_args(config: Mapping[str, Any], data_root: Path) -> dict[str, Any]:
    ...
    return {
        "day_of_week_source": str(config.get("day_of_week_source", ORIGINAL_DAY_SOURCE)),
        "weekday_mapping": mapping,
        "all_days": [str(item) for item in manifest.get("all_days", [])],
    }


def assemble_batch_history(
    ...
    *,
    ...
    return assemble_history_features(
        batch["x"],
        window_start_slot=batch["window_start_slot"],
        day_index=batch["day_index"],
        n_his=int(config.get("input_len", config.get("n_his", 12))),
        if_time_in_day=bool(config.get("if_T_i_D", config.get("if_time_in_day", True))),
        if_day_in_week=bool(config.get("if_D_i_W", config.get("if_day_in_week", True))),
...
def identity_indices_from_batch(
    batch: Mapping[str, torch.Tensor],
    *,
    day_of_week_source: str,
    weekday_mapping: Mapping[str, int] | None,
    ...
    """Integer identities: last history slot and original sequential weekday."""
    payload: dict[str, torch.Tensor] = {}
    if if_time_in_day:
        payload["time_of_day_index"] = batch["target_slot"].to(dtype=torch.long)
    if if_day_in_week:
        dow = day_of_week_index(
            batch["day_index"].detach().cpu().numpy(),
            source=day_of_week_source,
            weekday_mapping=weekday_mapping,
            all_days=all_days,
        )
        payload["day_of_week_index"] = torch.as_tensor(
            dow, dtype=torch.long, device=batch["day_index"].device
        )
    return payload
...
def identity_kwargs(
    ...
    return identity_indices_from_batch(
        batch,
        day_of_week_source=str(day["day_of_week_source"]),
        weekday_mapping=day.get("weekday_mapping"),
        all_days=day.get("all_days"),
        if_time_in_day=bool(config.get("if_T_i_D", config.get("if_time_in_day", True))),
        if_day_in_week=bool(config.get("if_D_i_W", config.get("if_day_in_week", True))),
    )
...
def build_optimizer(model: torch.nn.Module, config: Mapping[str, Any]) -> torch.optim.Optimizer:
    ...
    if name != "adam":
        raise ReimplementationError(f"original PeMS STID optimizer is Adam, got {name}")
    return Adam(
        model.parameters(),
...
def build_scheduler(
    ...
    if name not in {"multisteplr", "multi_step", "multistep"}:
        raise ReimplementationError("original PeMS STID scheduler is MultiStepLR")
    milestones = [int(item) for item in config.get("lr_milestones", [1, 50, 80])]
    return MultiStepLR(
        optimizer,
...
def build_model(config: Mapping[str, Any]) -> STID:
    validate_no_graph_config(config)
    return STID.from_config(config)
...
def _gradient_flags(model: STID) -> dict[str, bool]:
    flags = {
        "time_series_embedding": False,
        "spatial_identity": False if model.if_spatial else True,
        "time_of_day_identity": False if model.if_time_in_day else True,
        "day_of_week_identity": False if model.if_day_in_week else True,
        "mlp_block": False,
        "regression_layer": False,
    }
    for name, tensor in model.named_parameters():
        ...
        if max_abs <= 0.0:
            continue
        if "time_series_emb_layer" in name:
            flags["time_series_embedding"] = True
        if name.startswith("node_emb") or name == "node_emb":
            flags["spatial_identity"] = True
        if "time_in_day_emb" in name:
            flags["time_of_day_identity"] = True
        if "day_in_week_emb" in name:
            flags["day_of_week_identity"] = True
        if "encoder." in name:
            flags["mlp_block"] = True
        if "regression_layer" in name:
            flags["regression_layer"] = True
    required = {
        "time_series_embedding": True,
        "mlp_block": True,
        "regression_layer": True,
        "spatial_identity": model.if_spatial,
        "time_of_day_identity": model.if_time_in_day,
        "day_of_week_identity": model.if_day_in_week,
    }
...
def run_p70_smoke(
    ...
    *,
    ...
    probe_history = assemble_batch_history(probe, config, **day).to(device)
    ids = identity_kwargs(probe, config, day)
    shape_info = validate_model_shapes(
        model,
        batch_size=int(probe_history.size(0)),
        history=probe_history,
        **ids,
    )
    optimizer = build_optimizer(model, config)
    criterion = MAERawLoss(float(data_info["mean_y_full"]), float(data_info["std_y_full"]))
    model.train()
    pred, trace = model(probe_history, return_trace=True, **ids)
    y = probe["y"]
    ...
    if bool(config.get("clip_grad_norm", True)):
        torch.nn.utils.clip_grad_norm_(model.parameters(), float(config.get("max_grad_norm", 5.0)))
    optimizer.step()
    changed = 0
    for name, tensor in model.named_parameters():
    ...
    payload = {
        ...
        "history_shape": list(probe_history.shape),
        ...
        "if_spatial": model.if_spatial,
        "if_time_in_day": model.if_time_in_day,
        "if_day_in_week": model.if_day_in_week,
    ...
    print(
        f"[stid] stage=smoke_testing rate=p70 loss={payload['training_loss']:.6f} status=ok",
        flush=True,
    )
    del model
    del optimizer
    ...
    *,
    ...
    if resume_path is not None:
        ...
        assert_checkpoint_compatible(
            ...
            num_nodes=int(config["num_nodes"]),
            n_his=int(config.get("input_len", 12)),
            output_steps=1,
            input_channels=1,
            output_channels=1,
        )
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch in loaders_pack["loaders"]["train"]:
            ...
            history_data = assemble_batch_history(batch, config, **day).to(device)
            pred = model(history_data, **identity_kwargs(batch, config, day))
    ...
    return {"rate": tag, "best_mae": best_mae, "epochs": epochs, "init_reset_seed": seed}
