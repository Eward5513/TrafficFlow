"""Huber / Smooth-L1 matching ``reference/STFGNN/models/stsgcn_4n_res.py::huber_loss``.

Original training compares the network output with the data iterator's ``label``.
That repository yields normalized ``x`` and **raw** ``y``, so Huber is computed
on the original (vehicle-count) scale with ``rho=1``.
